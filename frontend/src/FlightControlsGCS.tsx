...
const Telem: React.FC<{ state: any; socket: Socket }> = ({ state, socket }) => {
  ...
  useEffect(() => {
    const telemUpdater = (data: any) => {
      ...
      if (Date.now() - (state.current.hudPaint || 0) > 120) { state.current.hudPaint = Date.now(); refreshHud((value) => value + 1); }
    };
  ...
  return (
    <>
      <div className="hud-shell">
        ...
        <div className="flight-reticle" aria-label="primary flight display">
          <div className="roll-scale"><i /><i /><i /><i /><i /></div>
          <div className="horizon-bank" style={{ transform: `rotate(${-roll}deg)` }}>
            <div className="pitch-ladder" style={{ transform: `translateY(${pitch * 2}px)` }}>
              <span data-pitch="20" /><span data-pitch="10" /><span className="zero-line" /><span data-pitch="-10" /><span data-pitch="-20" />
            </div>
          </div>
          <div className="aircraft-symbol"><b /><i /><b /></div>
          <div className="heading-bug">HDG {yaw.toFixed(0).padStart(3, "0")}°</div>
        </div>
        ...
        <div className="gcs-footer">
          <span><i className="live-dot" /> VEHICLE 01</span>
          <span>LAT {state.current?.lat ? Number(state.current.lat).toFixed(5) : "—"}</span>
          <span>LON {state.current?.lon ? Number(state.current.lon).toFixed(5) : "—"}</span>
          <span>LINK {connectionAge < 2 ? "NOMINAL" : "LOST"}</span>
        </div>
      </div>

      <div className="control-pad left">
        <div className="control-pod-title"><span>THR / YAW</span><small>W S · A D</small></div>
      ...
      <div className="control-pad right">
        <div className="control-pod-title"><span>PITCH / ROLL</span><small>ARROW KEYS</small></div>
        <div className="joystick-shell">
          <Joystick
